...
network_graph <- function (kne, type="definition") {
    ...
    results <- lapply(seq_along(raw_nodes$name), function(nn) {
        ...
        if (is.character(gra)) {es <- NULL; vs <- NULL;} else {
			...
			vs[,1] <- ifelse(startsWith(vs[,1],"manual_G"),
				paste0(vs[,1],"_",nn,"_",kne@ID) ,vs[,1])
			vs <- do.call(rbind,
				lapply(vs[,1], function(j) {c(j, bln, "in_block")}))
        }
    ...
    edges <- do.call(rbind, lapply(results, function(x) x[["es"]]))
	name_change <- paste0("manual_BLOCK",results[["name_change"]],"_",kne@ID)
    names(name_change) <- as.character(results[["nns"]])
    
    new_edges_from <- lapply(raw_edges$from, function(i) {
    	if (i %in% names(name_change)) {
            return(as.character(name_change[as.character(i)]))
        } else {
            return(raw_nodes$name[i])
        }
    }) |> unlist()
	...
	all_edges <- rbind(
		raw_edges |> `colnames<-`(c("from","to","type","subtype")),
        edges
    )
